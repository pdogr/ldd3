...
static int trim(scullpa_dev *);
static int follow(scullpa_dev *, struct qset **, int qsetll_off);

static int err = 0;
static int major = 0, minor = 0, quantum = PAGESIZE * (1 << SCULLPA_ORDER),
           qset_size = SCULLPA_QSET_SIZE;
static scullpa_dev *devs;
...
static int trim(scullpa_dev *dev) {
  int i = 0;
  struct qset *cur, *tmp;
  if (!dev) {
    return 0;
  ...
  while (cur) {
    if (cur->data) {
      for (i = 0; i < dev->qset_size; ++i) {
        if (cur->data[i]) {
          free_pages((long unsigned int)cur->data[i], SCULLPA_ORDER);
        }
      }
      kfree(cur->data);
      cur->data = NULL;
...
static int follow(scullpa_dev *dev, struct qset **qset, int qsetll_off) {
  struct qset *cur = 0;
  cur = dev->qset;
  if (!cur) {
...
int open(struct inode *inode, struct file *filp) {
  scullpa_dev *dev;
  dev = container_of(inode->i_cdev, scullpa_dev, cdev);
...
ssize_t write(struct file *filp, const char __user *buff, size_t count,
  ...
  if (!qset->data[qset_off]) {
    if (!(qset->data[qset_off] =
              (void *)__get_free_pages(GFP_KERNEL, SCULLPA_ORDER))) {
...
static void exit_m(void) {
  ...
  remove_proc_entry("scullpa", 0);
  PDEBUG("scullpa: exit module");
  unregister_chrdev_region(MKDEV(major, minor), SCULLPA_DEVS);
}

static int __init init_m(void) {
  int r, i;
  dev_t _dev = 0;
  r = alloc_chrdev_region(&_dev, minor, SCULLPA_DEVS, "scullpa");
  major = MAJOR(_dev);
  if (r < 0) {
    return r;
  }
  if (!(devs = kmalloc(SCULLPA_DEVS * sizeof(scullpa_dev), GFP_KERNEL))) {
    r = -ENOMEM;
    goto init_end;
  }
  memset(devs, 0, SCULLPA_DEVS * sizeof(scullpa_dev));
  for (i = 0; i < SCULLPA_DEVS; ++i) {
    scullpa_dev *dev = &devs[i];
    dev->quantum = quantum;
    dev->qset_size = qset_size;
    dev->size = 0;
    mutex_init(&dev->lock);
  ...
  PDEBUG("Quantum: %d, Qset_size: %d", quantum, qset_size);
  proc_create("scullpa", 0, 0, &proc_fops);
  PDEBUG("scullpa: init success");
  return 0;
init_end:
  PDEBUG("scullpa: init Failure");
  unregister_chrdev_region(MKDEV(major, minor), SCULLPA_DEVS);
  return r;
}
